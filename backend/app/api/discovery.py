...
def discover(
    ...
    server.system_hostname = result.fqdn

    if server.use_system_hostname:
        server.name = (
            result.fqdn
            if server.use_fqdn
...
@router.post(
    "/{server_id}/hostname-preview",
    response_model=HostnamePreviewResponse,
)
def hostname_preview(
    ...
    return HostnamePreviewResponse(
